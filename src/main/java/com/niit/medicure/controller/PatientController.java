...
public class PatientController {
   ...
   @GetMapping("/welcomeDoctor")
	...
	public String bookAppoint(HttpServletRequest req,@ModelAttribute("appForm") Appointment app,Model theModel) {
	 ...
	 appService.save(app);
	 Appointment a1=appService.findByAppid(app.getAppid());
	 Doctor d=doctorService.get(a1.getDoctorid());
	 app.setDoctorname(d.getName());
	 //app.setDoctoraddress(d.getDoctoraddress());
	 appService.save(app);
	 Appointment appbooked=appService.findByAppid(app.getAppid());
	 req.getSession().setAttribute("appbooked",appbooked);
	...
	@PostMapping("/save")
	public String viewLogin(HttpServletRequest req,Model theModel,@ModelAttribute("doctor")Doctor theDoc)
	{
		Doctor d=(Doctor)req.getSession(false).getAttribute("doctor");
		d.setPhoneNum(theDoc.getPhoneNum());
		
		doctorService.save(d);
		 return "redirect:/loginUs/editDoctor";
	}
...
public String viewGateway(HttpServletRequest req,@ModelAttribute("payForm") Appointment app,Model theModel) {
	Patient p=(Patient)req.getSession(false).getAttribute("patient");
	Appointment a=(Appointment)req.getSession(false).getAttribute("appbooked");
	theModel.addAttribute("paydetail", a);
...
public String viewPatientAppDetailsFromHistory(HttpServletRequest req,Model theModel,@RequestParam("app_id") int appid) {
	...
	theModel.addAttribute("pres",a1);
	Patient p2=(Patient)req.getSession(false).getAttribute("patient");
	theModel.addAttribute("forpresedit",p2);
	Patient p1=patientService.get(a1.getPatientId());
	theModel.addAttribute("presBasic",p1);
...
public String savePatient(HttpServletRequest req,@ModelAttribute("patient") Patient thePat,Model theModel)
{ 
	
	Patient p=(Patient)req.getSession(false).getAttribute("patient");
	p.setName(thePat.getName());
	p.setEmail(thePat.getEmail());
	p.setPhoneNum(thePat.getPhoneNum());
	patientService.save(p);
	return "redirect:/loginUs/edit";
	
}
/*
@RequestMapping(value = "/savePatient",method = {RequestMethod.GET,RequestMethod.POST})
